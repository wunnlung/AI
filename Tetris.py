...
def fitness(chromosome):
    ...
    fitness_scores = [play(pygame.display.set_mode((s_width, s_height)), chromosome) for _ in range(10)]
...
def fitnessFile(s):
    ...
    best_fitness = max(s)
    average_fitness = sum(s) / len(s)
    #add the current generation count and best performance of the generation to the file
    with open("training.txt", "a") as f:
        f.write(f"Generation: {current_generation}, Best Fitness: {best_fitness} Average Fitness: {average_fitness:.2f}\n")
...
def play(window, chromosome):
    ...
    ####################################################################
    #    highest_blocks = [None] * 10  #init with None, so that we can keep same format
    #    for x in range(10):  #10 cols
    #        for y in range(20):  # 20 rows
    #            if grid[y][x] != (0, 0, 0): #empty checker
    #                highest_blocks[x] = 20 - y #doing 20 - y to reverse the number
    #                break

        #print("Highest blocks in each column:", highest_blocks)
    ####################################################################
